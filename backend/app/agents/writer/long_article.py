...
def long_article_writer(state: dict) -> dict:
    """
    长篇 Writer - 900-1800字深度分析
    约束已烘焙到模板中，无需运行时注入
    """
    ...
    # 获取模式配置
    mode_config = get_mode_config("long_article")
    length_constraints = mode_config.get("length", {"min": 900, "max": 1800, "target": 1200})
    ...
    if get_feature_flag("use_knowledge_repo"):
        web3_knowledge = retrieve_web3_knowledge(raw_input[:200])
    ...
    if custom_prompts.get("writer"):
        ...
        system_prompt = render_modular_prompt("writer/long_article.jinja2", context)
...
请现在开始撰写深度分析文章。【重要：必须使用中文撰写！】"""
    ...
    max_tokens = min(int(length_constraints.get("max", 1800) * 1.5), 8192)
