...
### 阶段 2: P1 ConfigPanel + DataPanel 重构

> **目标**: 实现 switch(mode) 动态面板 + DataPanel + 状态联动 + 3 个新模式的前端 shell。
> **预估**: 6-8 小时
> **依赖**: P0 完成后
> **终审反馈**: [P27_架构设计终审反馈.md](file:///d:/AI_Projects/2026001/reports/设计文档/P27_架构设计终审反馈.md) 中 3 条采纳项已纳入
...
| **8** | **⭐ `useAgentStore` 新增 `selectedMaterial` + `isDirty`，三面板状态联动**（终审反馈 #2） |
| **9** | **⭐ DataPanel 加 Skeleton 骨架屏 + Empty State 空状态**（终审反馈 #3） |
| **10** | **⭐ `setMode()` 模式切换守卫：自动保存草稿 + Toast 提示**（终审反馈 #4） |
...
| 架构终审反馈 + 团队结论 | [P27_架构设计终审反馈.md](file:///d:/AI_Projects/2026001/reports/设计文档/P27_架构设计终审反馈.md) |
