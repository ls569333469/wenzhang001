...
### 遗留 narrative_type 清理

| narrative_type | 操作 | 文件 |
|:---|:---:|:---|
| `project_review` | ✅ 保留 | 默认值，覆盖所有模式 |
| `tutorial` | ✅ 保留 | 对应教程模式 |
| `market_news` | ❌ 删除 | strategist.py, writer.py, 模板 |
| `opinion` | ❌ 删除 | strategist.py, writer.py, 脚本 |
| ~~`micro_novel`~~ | ✅ 已删除 | - |
...
> [!IMPORTANT]
> **hot_take 独立 API 处理** ✅ 已确认选项 A
> - ✅ **选项 A**: 保持独立，不纳入 writer/ 模块
> - ~~选项 B~~: 统一到 writer/hot_take.py
...
- [ ] 同意 8.5h 工时估算
- [ ] 同意迁移预案和回滚策略
- [x] 确认 hot_take API 处理方案 (**选项 A: 保持独立**)
