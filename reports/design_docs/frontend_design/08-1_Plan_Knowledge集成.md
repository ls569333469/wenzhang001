...
> **方案**: Feature Flag 方式 (A/B 测试)，新旧流程共存  
> **当前阶段**: 🔄 A/B 测试中  
> **更新日期**: 2026-01-24
...
| 里程碑 | 状态 | 完成日期 | 备注 |
|--------|------|----------|------|
| 后端 Feature Flag 配置 | ✅ 完成 | 2026-01-23 | `use_knowledge_repo` 开关可用 |
| Knowledge 检索服务 | ✅ 完成 | 2026-01-23 | `knowledge_retriever.py` 实现 |
| Strategist 集成 | ✅ 完成 | 2026-01-23 | 可注入 `web3_knowledge` |
| 前端开关 UI | ✅ 完成 | 2026-01-23 | Settings 页面可切换 |
| 字段规范迭代 | ✅ v4 定稿 | 2026-01-24 | 10 字段（移除来源链接等） |
| 数据重新入库 | ⏳ 待执行 | - | 表已清空，待入库 |
| Writer 集成 | ⏳ 待实现 | - | 待检索质量提升后实现 |
| 检索质量优化 | ⏳ 规划中 | - | 考虑语义匹配(embedding) |

### 已知问题 (来自测试报告)

| 问题 | 影响 | 建议解决方案 |
|------|------|--------------|
| 检索逻辑过于简单 | 不相关内容被检索 | 引入语义匹配 |
| Knowledge_Repo 数据单一 | 仅 DAO 相关内容 | 扩充数据源 |
| Writer 未使用检索结果 | Feature 未完整 | 待检索质量提升后实现 |
