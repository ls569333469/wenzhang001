...
| 组件 | 技术 |
|------|------|
| 前端页面 | Next.js `/knowledge` 路由 |
| 状态管理 | React useState + SSE |
| 后端 API | FastAPI `/api/ingest/batch` |
| 进度推送 | Server-Sent Events (SSE) |

### 12.4 API 设计

```
POST /api/ingest/start       # 开始批量入库
POST /api/ingest/pause       # 暂停当前批次
POST /api/ingest/resume      # 继续下一批次
GET  /api/ingest/status      # 获取执行状态 (SSE)
GET  /api/ingest/history     # 获取历史执行记录
```
